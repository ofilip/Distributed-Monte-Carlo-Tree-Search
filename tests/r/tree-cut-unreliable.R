...
if (export) {
	setEPS()
	postscript(file="../text/img/tree-cut-unreliable.eps", width=6, height=4.5)
}
...
#distr.data <- read.delim("results/20130624-1007-tree-cut-reliability.txt", row.names=NULL)
distr.data <- read.delim("results/20130626-0311-tree-cut-reliable.txt", row.names=NULL)

distr.data2 <- distr.data[c(wanted_cols,"reliability","ghost_sims_per_sec","ru_prob")]
