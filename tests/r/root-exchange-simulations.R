...
if (export) {
	setEPS()
	postscript(file="../text/img/root-exchange-simulations.eps", width=6, height=9)
}

# reference plain MCTS data
plain_p.data <- read.delim("results/20130530-1000-plain-mcts-pesimistic.txt", row.names=NULL)
plain_p.data2 <- plain_p.data[c(wanted_cols, "ghost_sims_per_sec")]
...
distr.data2 <- distr.data[c(wanted_cols, "channel_speed", "sims_per_sec_calculated", "sims_per_sec_total","ghost_avg_decision_sims",
	"transmitted_per_second_successfully")]
...
plot(log(distr.scores20$channel_speed), distr.scores20$sims_per_sec_calculated,
	type="o", axes=TRUE)
lines(log(distr.scores40$channel_speed), distr.scores40$sims_per_sec_calculated,
	type="o")
axis(2, at=seq(0.0,4.0,0.5))
axis(1, at=log(2^seq(4,18,2)), labels=2^seq(4,18,2))
box()
